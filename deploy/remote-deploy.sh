...
OBJECT_STORAGE_KEY_NAME=$OBJECT_STORAGE_KEY_NAME
OBJECT_STORAGE_ACCESS_KEY=$OBJECT_STORAGE_ACCESS_KEY
OBJECT_STORAGE_SECRET_KEY=$OBJECT_STORAGE_SECRET_KEY
OBJECT_STORAGE_PATH_STYLE=$OBJECT_STORAGE_PATH_STYLE
SEED_TEST_USER_EMAIL=$SEED_TEST_USER_EMAIL
SEED_TEST_USER_PASSWORD_B64=$SEED_TEST_USER_PASSWORD_B64
SEED_TEST_USER_FIRST_NAME=$SEED_TEST_USER_FIRST_NAME
SEED_TEST_USER_LAST_NAME=$SEED_TEST_USER_LAST_NAME
...
if [[ "$DEPLOY_WEBAPP_ONLY" == "true" ]]; then
  ...
  if ! docker compose "${COMPOSE_ENV_FILES[@]}" up -d db garage; then
    echo "Error: docker compose failed to start db/garage."
...
ensure_garage_bucket_and_key() {
  local garage_container="lehrerlog-${ENV_NAME}-garage"
  local key_name="${OBJECT_STORAGE_KEY_NAME:-lehrerlog-${ENV_NAME}-server}"
  local bucket_name="$OBJECT_STORAGE_BUCKET"
  local output=""
  local key_id=""
  local secret_key=""
  ...
  if [[ -z "$bucket_name" ]]; then
    echo "Error: OBJECT_STORAGE_BUCKET is required."
    return 1
  fi
  ...
  if docker exec -e GARAGE_CONFIG_FILE=/etc/garage.toml "$garage_container" /garage key info "$key_name" >/dev/null 2>&1; then
    key_exists="true"
  fi
  ...
  if [[ -z "$OBJECT_STORAGE_ACCESS_KEY" || -z "$OBJECT_STORAGE_SECRET_KEY" ]]; then
    ...
    output="$(docker exec -e GARAGE_CONFIG_FILE=/etc/garage.toml "$garage_container" /garage key create "$key_name" 2>/dev/null || true)"
    key_id="$(printf '%s\n' "$output" | sed -n -E 's/^Key ID:[[:space:]]*//p' | head -n 1)"
    secret_key="$(printf '%s\n' "$output" | sed -n -E 's/^Secret key:[[:space:]]*//p' | head -n 1)"
    if [[ -z "$key_id" || -z "$secret_key" ]]; then
      echo "Error: Failed to create Garage key."
      docker logs "$garage_container" --tail 200 || true
      return 1
    fi
    OBJECT_STORAGE_KEY_NAME="$key_name"
    OBJECT_STORAGE_ACCESS_KEY="$key_id"
    OBJECT_STORAGE_SECRET_KEY="$secret_key"
    key_exists="true"
  fi

  if ! docker exec -e GARAGE_CONFIG_FILE=/etc/garage.toml "$garage_container" /garage bucket list 2>/dev/null | grep -q "$bucket_name"; then
    docker exec -e GARAGE_CONFIG_FILE=/etc/garage.toml "$garage_container" /garage bucket create "$bucket_name" >/dev/null
  fi

  if [[ "$key_exists" == "true" ]]; then
    docker exec -e GARAGE_CONFIG_FILE=/etc/garage.toml "$garage_container" /garage bucket allow \
      --read --write --owner "$bucket_name" --key "$key_name" >/dev/null || true
  fi
}

if [[ "$DEPLOY_WEBAPP_ONLY" != "true" ]]; then
  init_garage_layout || exit 1
  ensure_garage_bucket_and_key || exit 1
  write_server_env
  docker compose "${COMPOSE_ENV_FILES[@]}" up -d --force-recreate lehrerlog-server
fi
