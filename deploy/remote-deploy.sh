...
COMPOSE_ENV_FILE="$SERVER_ENV_FILE"
...
deploy_webapp() {
  ...
  if [[ "$skip_pull" != "true" ]]; then
    docker compose --env-file "$COMPOSE_ENV_FILE" pull lehrerlog-webapp
  fi
  docker compose --env-file "$COMPOSE_ENV_FILE" up -d lehrerlog-webapp
...
if [[ "$DEPLOY_WEBAPP_ONLY" == "true" ]]; then
  ...
  docker compose --env-file "$COMPOSE_ENV_FILE" pull lehrerlog-server db garage
else
...
if [[ "$DEPLOY_WEBAPP_ONLY" == "true" ]]; then
  ...
  docker compose --env-file "$COMPOSE_ENV_FILE" up -d lehrerlog-server db garage
else
  docker compose --env-file "$COMPOSE_ENV_FILE" up -d --remove-orphans
fi

if [[ "$DEPLOY_WEBAPP_ONLY" != "true" ]]; then
  docker compose --env-file "$COMPOSE_ENV_FILE" up -d --force-recreate garage
fi
